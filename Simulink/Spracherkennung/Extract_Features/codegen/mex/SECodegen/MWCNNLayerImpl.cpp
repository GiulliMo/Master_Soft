...
pipeline[i].execute(s, pipeline_memory_args[i]); }
 ...
 reorderedLayerOutputTap(0);
#endif
 return; } void MWReLULayerImpl::allocateOutputData(int i) { MWTensor* ipTensor 
= getLayer()->getInputTensor(); MWTensor* opTensor = 
getLayer()->getOutputTensor(i); if (opTensor->getopBufIndex() < 0 && 
UEESbUvbMihFnquvuFij) { MWMkldnnUtils::getLayerMemoryPrimitive(opTensor, 
euppfEoiaoCTcVgRPVhA) ->set_data_handle(ipTensor->getData<float>()); 
