...
config.Window = obj.Window;
config.WindowLength = numel(obj.Window);
config.OverlapLength = obj.OverlapLength;
config.HopLength = numel(obj.Window);
config.FFTLength = obj.FFTLength;
config.Window = obj.SampleRate;
%%
%% outputIndex = obj.pOutputIndexReduced;
outputIndex.barkSpectrum = true;
%%
...
featureVector = zeros(numHops,100,numChan,'like',x); % config.FeaturesPerHop
