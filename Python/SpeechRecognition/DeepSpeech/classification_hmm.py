...
training_data.append({"class":3, "sentence":"localize yourself", "modus":2})
training_data.append({"class":3, "sentence":"localization", "modus":2})
...
training_data.append({"class":5,  "sentence":"i hear that a note", "modus":2})
...
for doc in documents:
    doclengths.append(len(doc[0]))

print(max(doclengths))
...
for doc in documents:
    ...
    for s in pattern_words:
        X1 = bow(s.lower(), words, show_details=False)
        features = np.concatenate([features, X1])
        lengths_list.append(349)

    i = len(pattern_words)
    for i in range(max(doclengths)-len(pattern_words)):
        features = np.concatenate([features, zeros])
        lengths_list.append(349)


    lengths.append(lengths_list)
...
model = hmm.GaussianHMM(n_components=1, covariance_type="full", n_iter=5)

model = model.fit(X[0:50], lengths[0:50])
...
#x, Z = model.sample(len(words))

sentence1 = "localize yourself in unknown environment"
sentence_words = clean_up_sentence(sentence1)
features = []
for s in sentence_words:
    X1 = bow(s.lower(), words, show_details=False)
    features = np.concatenate([features, X1])
...
#z = model.predict_proba(input)
#print(z)
#print(model.decode(input))
#print(model.score(input))