...
class SpeechRecognition:
    ...
    def getAlfBuzzWords(self):
        ...
        if self.transcript:
            ...
            my_string=''
            for i in range(len(self.recognizedBuzzwords)):
                my_string = my_string.join(self.recognizedBuzzwords[i]['buzzword'][0]['name'])
    ...
    def classifierPostProcessing(self, transcript, classified, recognizedBuzzwords):
        ...
        if classified == self.class_names[0]:

            if not recognizedBuzzwords:
                #ausgabe von sprache
                print("no valid location") #tts
            elif recognizedBuzzwords:

                print("valid")
                #veroeffentlichen eines ziels
                # ausgabe von sprachetts
